...
# DRIVER_INSTALLATION_PATH = GeckoDriverManager().install()
DRIVER_INSTALLATION_PATH = "/usr/bin/geckodriver"
...
def look_up_ris_file(dates, folder):
    ...
    driver = webdriver.Firefox(firefox_profile=profile, service=Service(DRIVER_INSTALLATION_PATH), options=options)
...
def get_filtered_ris_file(folder, file_name):
    ...
    driver = webdriver.Firefox(firefox_profile=profile, service=Service(DRIVER_INSTALLATION_PATH), options=options)
    time.sleep(10)
    driver.get('http://robotsearch:5000')
